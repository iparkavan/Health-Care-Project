...
class ICD:
    ...
    def get_icd_data(self):
        with open(f"{data_folder}/icd_dump.json", 'r') as icd_file:
    ...
    def get_icd_data_dump(self):
        with open(f"{data_folder}/icd10_desc.json", 'r') as icd_file:
            icd_dict = json.load(icd_file)
        ...
        for name in names_2:
            ...
            for val in name.itersiblings():
                if val.tag == 'diag':
                    ...
                    for elem in val.iterchildren():
                        icd_dict[name.text][val.tag].append(elem.text)
        with open(f"{data_folder}/icd_dump.json", 'w') as icd_file:
